...
const slug = process.env.SMOKE_WORK_SLUG;

if (!slug || !(workInteractiveSlugs as readonly string[]).includes(slug)) {
  throw new Error(
    `SMOKE_WORK_SLUG must be one of: ${workInteractiveSlugs.join(', ')}`,
  );
}

test.describe('single work smoke', () => {
  test(`/works/${slug} mounts canvas, controls, metadata, and declared interaction`, async ({
    page,
  }) => {
    ...
    await exerciseDeclaredInteraction(
      page,
      slug as WorkInteractiveSlug,
      workInteractionHints[slug as WorkInteractiveSlug],
    );
...
async function exerciseDeclaredInteraction(
  page: Page,
  workSlug: WorkInteractiveSlug,
  hint: WorkInteractionHint,
) {
  const controls = page.locator(`#${workSlug}-controls`);

  if (hint === 'button') {
    const button = controls.locator('button:not([disabled])').first();
    await expect(button, `${workSlug} should expose a clickable button`).toBeVisible();
    await button.click();
    return;
  }
  ...
  if (hint === 'input') {
    ...
    await expect(input, `${workSlug} should expose an input control`).toBeVisible();
  ...
  if (hint === 'select') {
    ...
    await expect(select, `${workSlug} should expose a select control`).toBeVisible();
    await select.evaluate((element) => {
      const selectElement = element as HTMLSelectElement;
      const nextIndex =
        selectElement.selectedIndex + 1 < selectElement.options.length

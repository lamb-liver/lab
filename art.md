...
### 6.3 複數幾何主題（complex 系列）補充

複數平面三題（四則運算、極座標、尤拉旋轉）沿用同一視覺語言，僅替換 geometry 與互動參數。

| 主題 | 主體 | guide | UI |
|------|------|-------|----|
| 複數四則運算 | `z1`、`z2`、`z1+z2`、`z1*z2` 向量 glow | 平行四邊形輔助邊 + 軸線低 alpha | 左上角僅顯示 r/θ 與運算標記 |
| 複數極座標 | 單一向量 `z=r e^{i\theta}` glow | Re/Im 投影虛線 + `θ` 弧線 | 小字標示 `z=a+bi`、`r`、`θ` |
| 尤拉旋轉 | 旋轉向量 + 右側時域波形 glow | 端點到波形起點投影虛線 + 低亮度軸線 | 僅顯示 `A, ω, δ` |

落地要求：

- 依 §2 層級：主向量/曲線必須永遠強於 guide/projection。
- 依 §3 token：沿用 `bg.canvas` + `color.accent`，避免新增第二主色。
- 依 §4.2：所有投影與輔助線 `noFill()` + 低 alpha。
- 依 §5：不加 panel 框，僅保留低對比文字。

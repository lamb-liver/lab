...
- **交於一點／平行不相交／直線落在平面上**：切換三個預設，對照 $\mathbf n\cdot\mathbf d$ 與 $\mathbf n\cdot\mathbf r_0-h$ 的組合
- **平面傾角**：旋轉法向 $\mathbf n$，觀察 $\mathbf n\cdot\mathbf d$ 何時穿過 $0$，交點隨之消失
- **平面位移 h**：平移平面，觀察交點沿直線滑動，$t$ 的正負決定它落在起點哪一側
- **直線仰角**：改變 $\mathbf d$ 的方向，比較與平面斜交、相切的差別
- **拖動畫面旋轉視角**：旋轉場景，確認交點確實同時落在直線與平面上；也可改用視角水平與視角仰角滑桿

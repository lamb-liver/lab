...
- **模式切換**：成長／衰減兩種符號，$C$ 與 $|k|$ 滑桿
- **時間掃描**：$t$ 推進，曲線 reveal 並標記當前 $y(t)$
- **切線斜率**：顯示 $\dfrac{dy}{dt}=ky$，強調斜率與高度成正比
- **對數尺度連結**：可選切換縱軸為 $\ln y$，成長變直線（連結對數尺度作品）

## 相關連結

- 相關作品：[對數尺度](/works/logarithmic-scale)、[自然對數 e 的幾何定義](/works/natural-log-e-geometry)、[邏輯斯諦曲線](/works/logistic-curve)

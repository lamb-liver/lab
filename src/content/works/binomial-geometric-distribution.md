...
二項分佈 $\mathrm{B}(n,p)$：$n$ 次獨立試驗、每次成功機率 $p$，成功次數 $X\in\{0,1,\ldots,n\}$
...
幾何分佈 $\mathrm{Geo}(p)$：每次試驗成功機率 $p$，$X$ 為第一次成功前的失敗次數，$X\in\{0,1,2,\ldots\}$
...
本作品採用「第一次成功前的失敗次數」版本，因此幾何分佈的取值從 $0$ 開始。若以「第一次成功所需試驗次數」定義，取值會改為 $\{1,2,\ldots\}$，且 $E(X)=1/p$。

## 互動說明

- **分佈切換**：在二項與幾何之間切換，長條圖與公式區同步更新
- **參數 p**：拖動成功機率 $p$，觀察長條形狀與 $E(X)$、$\mathrm{Var}(X)$ 變化
- **試驗次數 n**：二項模式下調整 $n$，觀察長條個數與峰值位置
- **重心與展寬**：疊加 $E(X)$ 標記與 $\pm\sigma$ 區間，對照兩種分佈的集中程度

## 觀察重點

- 二項分佈有上界 $n$，長條只在 $0$ 到 $n$；幾何分佈尾端可延伸到很大取值，右尾較長。
- $p$ 增大時，二項分佈的 $E=np$ 線性增大；幾何分佈採「第一次成功前的失敗次數」版本時，$E=(1-p)/p$ 則非線性減小。

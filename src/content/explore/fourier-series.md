...
各係數 $a_n,b_n$ 由 $f$ 與正交三角函數的內積決定；有限項截斷即為對 $f$ 的近似。

## 互動說明

- **項數 N**：以滑桿調整疊加項數，觀察圓周運動（epicycles）如何逼近方波或 2D 週期軌道
- **1D／2D 模式**：切換一維波形與二維軌跡，對照同一組諧波在兩種呈現下的差異
- **吉布斯現象**：在不連續處，項數增加仍留下約 9% 跳躍高度的振盪，誤差不會完全消失

建議順序：少量諧波 → 增加 N 觀察貼合 → 對照不連續邊緣的吉布斯波峰。

## 觀察重點

- 再複雜的週期軌跡都可拆成多個基本頻率（圓運動）的疊加
- 函數愈平滑，係數衰減愈快，有限 N 的近似愈快貼近目標
- 訊號處理中的頻率分解，與幾何上「圓套圓」描繪是同一套數學

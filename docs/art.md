...
| `/exam` 試題視覺化、Exam 靜態封面 | [`exam-visualization-plan.md`](exam-visualization-plan.md) |
| `/contest` 競賽研題草稿、Contest 靜態封面 | [`architecture.md`](architecture.md)；目前只依現有 slug 實作，不抽成公開視覺規格 |
...
- Works runtime 線寬與 Explore／Exam／Contest 靜態封面線寬屬於不同輸出尺度，不互相套用。
...
- 新增或調整 Exam：先讀 [`exam-visualization-plan.md`](exam-visualization-plan.md)，再讀 [`p5toreact.md`](p5toreact.md)。
- Contest 目前保持草稿；調整現有研究時讀 [`architecture.md`](architecture.md)、slug-specific CSS 與 [`p5toreact.md`](p5toreact.md)，不要從單一頁面推導通用視覺系統。

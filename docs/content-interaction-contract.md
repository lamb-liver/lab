...
當然要在平面上，這種錯誤只有目視才會發現；線性規劃那張第一版的約束線沒有通過它所圍出的角點，
也是一樣。

**而且要縮到卡片大小、和既有封面排在一起看**，不是單獨看大圖。新封面單獨看沒問題，
與鄰居並排才會發現撞衫——`complex-euler-formula` 與 `trig-function-graphs` 就是這樣
長成同一張圖的。作法見 `frontend-validation.md` §用 contact sheet 檢查。

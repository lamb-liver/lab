...
新增互動作品可用 `npm run new:work -- <slug> --interactive` 一次生成 module 骨架、Root 元件、module 測試骨架並接好三個 registry（content 草稿已存在時會保留原檔只補缺件；draft 階段即可接線；發布前替換佔位幾何與文案）。手動接線時同步三處：

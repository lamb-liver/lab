...
這個專案真正的進步，不只是從 19 件 Works 增長成三個穩定內容集合，而是逐步學會：**先決定內容角色，再沿既有架構做一條完整垂直切片，最後用數學、真實互動、手機畫面與發布 gate 一起驗收。**

最大的彎路則幾乎都遵循同一模式：先擴張、先清理或先自動化，之後才發現產品邊界、真實瀏覽器行為或驗證方法尚未定義。

## 目前基準

- 歷史分析範圍：219 個 `main` 提交，至 `e50a5e6`；後續決策更新至 2026-08-08。
- 內容：69 Works、21 Explore、11 Exam，共 101 篇。
- 產品線：Works、Explore、Exam。Contest Studies 已完整移除，不列為 backlog 或未來建議。
- 發布：採持續部署，production revision 以 `main` commit 為準。
...
| 8/4–8/8：新主題與產品線取捨 | iteration classifier 已避免把有限尾段直接叫作混沌 | 曾建立 Contest Studies 與 `homogeneous-normalization` 草稿 | Contest 與 iteration-dynamics 被包在同一個 13,888 行變更，review 成本過高；8/8 決定完整移除 Contest，除非使用者明確要求重啟，不再建議此方向 |
...
- **Concept／Learning Path**：發現與策展層，不是第四個內容集合，也不是完整課程系統。

這項定位收斂，是後面 registry、route、schema、封面與驗證能各自穩定的前提。

### 2. 從單頁實作變成可維護的渲染架構

- 純數學／幾何留在 `src/curve`、`src/explore`、`src/exam`。
- React root 管狀態與控制；renderer 只吃 snapshot；p5 lifecycle 集中在共用 hook。
- Works、Explore、Exam 各自有 registry 與 stage，不互相假共用。
...
- Works thumbnails 使用 build-time SVG；Explore／Exam 使用可重現的 SVG source → PNG pipeline。
...
| 為 `homogeneous-normalization` 建立獨立 Contest Studies | 不塞進 Explore 可以保住既有邊界 | 為單一草稿建立 schema、route、registry、封面與 audits，維護面大於已證明的產品價值 | 2026-08-08 完整移除；除非使用者明確要求重啟，不再提供此產品線建議 |
...
- public-page、release、art、site UX、math review、AGENTS 與 editing rules 已同步現行三集合邊界。
- TypeScript 規則已改成 current full `npm run typecheck` zero-error gate。
- Contest Studies 的 runtime、content、assets、tests、commands 與現行規格已移除。

後續原則：現行架構只分析 Works、Explore、Exam；歷史提及 Contest 時，只把它視為已終止的實驗。

### P0：移除 Contest Studies 產品線（已完成）

`homogeneous-normalization` 與整條 Contest Studies 產品線已完整移除，不保留草稿、隱藏 route 或發布建議。除非使用者明確說要重啟，未來規劃、審查與優化建議都不得重新提出此方向。
...
- `codex/seo-structured-data` 已重審；只移植 author、`og:locale` 與首頁 WebSite JSON-LD，放棄沒有現行缺陷依據的 section OG 產生器，移植後刪除舊分支。
...
1. 讀者要理解什麼？
2. 它屬於 Works／Explore／Exam 哪一個？為什麼？
3. 讀者唯一最重要的操作是什麼？
4. 畫面唯一最重要的可見結論是什麼？
...
- Explore 要證明它不是 Works 目錄；Exam 要證明它仍是高三考試準備。

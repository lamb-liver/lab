...
- 手機 `<1024px`：控制區包在 `<details.work-detail__controls>`，summary「調整參數」，**預設展開**；畫布正方形上限 `min(100%, 42vh, 420px)`。portal 目標 `<aside>` 收合時仍在 DOM，參數來自 React state 非 input 初值。有畫布拖／轉的頁 canvas `touch-action: none`；其餘維持 `pan-y`。
...
- 工程契約見 [`p5toreact.md`](p5toreact.md) 與 `src/lib/curveThumbnail.ts`。

...
| Runtime lifecycle | `src/components/curve/*`, `src/components/works/*`, `src/components/explore/*` | Preserve p5 mount/unmount and ref synchronization contracts. `useRectP5CanvasHost` mounts the sketch once; param/button changes go through `redrawKey` or `restartOn`. ResizeObserver must not call `resizeCanvas` when width/height/pixelDensity are unchanged (same-size write clears the bitmap). |
...
Exam synchronization lists live in [`architecture.md`](architecture.md). Use the canonical list instead of copying a Works or Explore pipeline.
...
- Are Works, Explore, and Exam boundaries still separate?

...
- [ ] 依教育部／國教院數學課綱列出本題實際使用的學習內容編碼，並確認 `unit` 年級、類別與觀念相符
- [ ] 不參考程式，僅用上述高中定義與公式獨立算到官方答案；交付紀錄須列出完整解題鏈與答案核對結果
- [ ] 逐項掃描學生可見的定義、公式、符號與推導，排除未列入該考科的進階概念；投影、渲染與程式算法不能成為理解前提
- [ ] 由只具高中數學知識的審查者從「先想一想」開始閱讀，能口述圖形變化、每一步公式來源與答案的關係
- [ ] `relatedExplore` / `relatedWorks` 的 slug 都已發布（非 draft）

每題交付時須留下以下紀錄，缺一項就視為未通過高中範圍驗證：
